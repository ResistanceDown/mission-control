...
export const updateTaskSchema = z.object({
  ...
  action: z.enum(['merge_to_main']).optional(),
...
export const habiTaskIngestSchema = z.object({
  ...
  items: z.array(
    z.object({
      ...
      agent_job: z.string().min(1).max(100).optional(),
      surface: z.string().min(1).max(100).optional(),
      execution_mode: z.enum(['audit_only', 'draft_pr']).optional(),
      allowed_tools: z.array(z.string().min(1).max(100)).max(20).optional(),
      input_artifacts: z.array(z.string().min(1).max(2000)).max(20).optional(),
      output_artifacts: z.array(z.string().min(1).max(2000)).max(20).optional(),
      mutation_mode: z.enum(['audit_only', 'artifact_only', 'repo_write']).optional(),
      handoff_target: z.string().max(200).nullable().optional(),
      runtime_model_policy: z.object({
        class: z.enum(['local', 'trinity', 'frontier']),
        fallbacks: z.array(z.string().min(1).max(100)).max(10),
      ...
      disposition: z.enum(['execute_now', 'founder_decision_needed', 'defer', 'narrative_only', 'reference_only']).optional(),
